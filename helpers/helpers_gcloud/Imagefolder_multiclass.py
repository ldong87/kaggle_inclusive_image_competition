...
def pil_loader(path, XMin=0., YMin=0., XMax=1., YMax=1.):
    # open path as file to avoid ResourceWarning (https://github.com/python-pillow/Pillow/issues/835)
    with open(path, 'rb') as f:
        img = Image.open(f)
        w, h = img.size
        # print(int(w*(XMax-XMin)), int(h*(YMax-YMin)))
        if XMin==0. and YMin==0. and XMax==1. and YMax==1.:
            return img.convert('RGB')
        else:
            return img.convert('RGB').crop([w*XMin, h*YMin, w*XMax, h*YMax])


def accimage_loader(path, XMin=0., YMin=0., XMax=1., YMax=1.):
    import accimage
    try:
        ...
        if XMin==0. and YMin==0. and XMax==1. and YMax==1.:
            ...
            return img.crop([w*XMin, h*YMin, w*XMax, h*YMax])
    except IOError:
        # Potentially a decoding problem, fall back to PIL.Image
        return pil_loader(path, XMin, YMin, XMax, YMax)


def default_loader(path, XMin=0., YMin=0., XMax=1., YMax=1.):
    from torchvision import get_image_backend
    if get_image_backend() == 'accimage':
        # print(path)
        return accimage_loader(path, XMin, YMin, XMax, YMax)
    else:
        # print(path)
        return pil_loader(path, XMin, YMin, XMax, YMax)
...
def make_dataset(dir, label_file, desc_file, extensions):
    ...
    images = list(zip(dir+label['ImageID']+'.jpg', label['label_code'].map(lambda x: class_to_idx[x]), label['XMin'], label['YMin'], label['XMax'], label['YMax'], label['weight_class']  ))
    weights = list(label['weight_class'])
    num_samples = label[['weight_class','label_code']].drop_duplicates(subset='label_code')['weight_class'].sum()
...
class DatasetFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        path, target, XMin, YMin, XMax, YMax, weight = self.samples[index]
        sample = self.loader(path, XMin, YMin, XMax, YMax)
