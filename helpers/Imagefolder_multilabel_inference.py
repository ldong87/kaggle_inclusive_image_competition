...
def make_dataset(dir, label_file, desc_file, extensions):
    ...
    images = list(zip(dir+label['ImageID']+'.jpg', label['label_code'].map(lambda x: [class_to_idx[i] for i in x.split(',')])  ))
    return images, classes, class_to_idx, idx_to_class, class_to_desc, weights_
...
class DatasetFolder(data.Dataset):
    ...
    def __init__(self, root, label_file, desc_file, extensions=IMG_EXTENSIONS, loader=default_loader, transform=None, target_transform=None):
        samples, classes, class_to_idx, idx_to_class, class_to_desc, weights_ = make_dataset(root, label_file, desc_file, extensions)
        if len(samples) == 0:
